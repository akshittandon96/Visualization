...
void draw_arrow_head(double sd[2], float se[2]);
...
InitGlui( void )
{
	...
	//GLUI_Rollout *rollout = Glui->add_rollout_to_panel(panel,  "Axes Color", 1 );
		...
		group = Glui->add_radiogroup_to_panel( rollout, &WhichColor );
			...
			comp_disp = Glui->add_panel("choose component to display");
			GLUI_Listbox *comp_list = Glui->add_listbox_to_panel(comp_disp, "Component", &display_component, -1, (GLUI_Update_CB)Choose_Object);
			comp_list->add_item(0, "Vector Magnitude");
			comp_list->add_item(1, "Angle");
			comp_list->add_item(2, "X Component");
			comp_list->add_item(3, "Y Component");
	...
	panel = Glui->add_panel(  "Choose object to open " );
			...
			obj_list->add_item (2, "vnoise");
			obj_list->add_item (3, "bruno3");
			obj_list->add_item (4, "cnoise");
	...
	Glui->add_checkbox("Arrow", &showArrow);
	Glui->add_checkbox("Color Plot", &colorPlot);
			...
			if (second_iteration == 0) {
				for (int i = 0; i < IMG_RES; i++) {
					for (int j = 0; j < IMG_RES; j++) {
						...
						int StreamlineVList[100][2];
						StreamlineVList[0][0] = next_i;
						StreamlineVList[0][1] = next_j;
						int iterations_fwd = 1;
						while (iterations_fwd < 100) {
							float vx = min_vx + (max_vx - min_vx) * vec_img[next_i][next_j][0] / 255.0;
							float vy = min_vy + (max_vy - min_vy) * vec_img[next_i][next_j][1] / 255.0;
							...
							if (next_i < 0 || next_j < 0 || next_i >= IMG_RES ||
								next_j >= IMG_RES) {
								break;
							}

							StreamlineVList[iterations_fwd][0] = next_i;
							StreamlineVList[iterations_fwd][1] = next_j;
							iterations_fwd++;
						}
						...
						for (int i = 0; i < iterations_fwd; i++) {

							sum[0] += noise_tex[StreamlineVList[i][0]][StreamlineVList[i][1]][0];
							sum[1] += noise_tex[StreamlineVList[i][0]][StreamlineVList[i][1]][1];
							sum[2] += noise_tex[StreamlineVList[i][0]][StreamlineVList[i][1]][2];
						}

						StreamlineVList[0][0] = i;
						StreamlineVList[0][1] = j;

						int iterations_bkwd = 1;
						while (iterations_bkwd < 100) {
							float vx = min_vx + (max_vx - min_vx) * vec_img[next_i][next_j][0] / 255.0;
							float vy = min_vy + (max_vy - min_vy) * vec_img[next_i][next_j][1] / 255.0;
							...
							StreamlineVList[iterations_bkwd][0] = next_i;
							StreamlineVList[iterations_bkwd][1] = next_j;
							iterations_bkwd++;
						}

						for (int i = 0; i < iterations_bkwd; i++) {
							sum[0] += noise_tex[StreamlineVList[i][0]][StreamlineVList[i][1]][0];
							sum[1] += noise_tex[StreamlineVList[i][0]][StreamlineVList[i][1]][1];
							sum[2] += noise_tex[StreamlineVList[i][0]][StreamlineVList[i][1]][2];
						}
						...
						LIC_tex[i][j][0] = sum[0] / (iterations_fwd + iterations_bkwd);
						LIC_tex[i][j][1] = sum[1] / (iterations_fwd + iterations_bkwd);
						LIC_tex[i][j][2] = sum[2] / (iterations_fwd + iterations_bkwd);
		...
		if (showArrow == 1) {
			...
			for (i = 0; i < this_poly->ntris; i++) {
				...
				for (j = 0; j < 3; j++) {
					...
					draw_arrow_head(new double[2]{ temp_v->x, temp_v->y }, new float[2]{ temp_v->vx, temp_v->vy });
...
for (int i = 0; i < this_poly->ntris; i++) {
	...
	case RAINBOW:
		...
		for (j = 0; j < 3; j++) {
			Vertex *temp_v = temp_t->verts[j];
			switch (display_component) {
			case 0:
				mag = sqrt((temp_v->vx)*(temp_v->vx) + (temp_v->vy)*(temp_v->vy));
				rainbow_color(mag, max_mag, min_mag, current_rgb);
				break;
			case 1:
				angle = (atan2(temp_v->vy, temp_v->vx) * 360) / (2 * M_PI);
				rainbow_color(angle, max_angle, min_angle, current_rgb);
				break;
			case 2:
				rainbow_color(temp_v->vx, max_vx, min_vx, current_rgb);
				break;
			case 3:
				rainbow_color(temp_v->vy, max_vy, min_vy, current_rgb);
				break;
			}
	...
	case BWR:
		...
		for (j = 0; j < 3; j++) {
			...
			case 0:
				mag = sqrt((temp_v->vx)*(temp_v->vx) + (temp_v->vy)*(temp_v->vy));
				bwr_color(mag, max_mag, min_mag);
				break;
			case 1:
				angle = (atan2(temp_v->vy, temp_v->vx) * 360) / (2 * M_PI);
				bwr_color(angle, max_angle, min_angle);
				break;
			case 2:
				bwr_color(temp_v->vx, max_vx, min_vx);
				break;
			case 3:
				bwr_color(temp_v->vy, max_vy, min_vy);
				break;
			}
			glColor3f(current_rgb[0], current_rgb[1], current_rgb[2]);
			glNormal3d(temp_v->normal.entry[0], temp_v->normal.entry[1], temp_v->normal.entry[2]);
	...
	case HEATMAP:
		...
		for (j = 0; j < 3; j++) {
			...
			case 0:
				mag = sqrt((temp_v->vx)*(temp_v->vx) + (temp_v->vy)*(temp_v->vy));
				heat1_color(mag, max_mag, min_mag);
				break;
			case 1:
				angle = (atan2(temp_v->vy, temp_v->vx) * 360) / (2 * M_PI);
				heat1_color(angle, max_angle, min_angle);
				break;
			case 2:
				heat1_color(temp_v->vx, max_vx, min_vx);
				break;
			case 3:
				heat1_color(temp_v->vy, max_vy, min_vy);
				break;
			}
			glColor3f(current_rgb[0], current_rgb[1], current_rgb[2]);
			glNormal3d(temp_v->normal.entry[0], temp_v->normal.entry[1], temp_v->normal.entry[2]);
	...
	case DISCRETE:
		...
		for (j = 0; j < 3; j++) {
			...
			case 0:
				mag = sqrt((temp_v->vx)*(temp_v->vx) + (temp_v->vy)*(temp_v->vy));
				discrete_color(mag, max_mag, min_mag);
				break;
			case 1:
				angle = (atan2(temp_v->vy, temp_v->vx) * 360) / (2 * M_PI);
				discrete_color(angle, max_angle, min_angle);
				break;
			case 2:
				discrete_color(temp_v->vx, max_vx, min_vx);
				break;
			case 3:
				discrete_color(temp_v->vy, max_vy, min_vy);
				break;
			}
			glColor3f(current_rgb[0], current_rgb[1], current_rgb[2]);
			glNormal3d(temp_v->normal.entry[0], temp_v->normal.entry[1], temp_v->normal.entry[2]);
	...
	case NONLINEAR_RAINBOW:
		...
		for (j = 0; j < 3; j++) {
			...
			case 0:
				mag = sqrt((temp_v->vx)*(temp_v->vx) + (temp_v->vy)*(temp_v->vy));
				nonlrainbow_color(mag, max_mag, min_mag);
				break;
			case 1:
				angle = (atan2(temp_v->vy, temp_v->vx) * 360) / (2 * M_PI);
				nonlrainbow_color(angle, max_angle, min_angle);
				break;
			case 2:
				nonlrainbow_color(temp_v->vx, max_vx, min_vx);
				break;
			case 3:
				nonlrainbow_color(temp_v->vy, max_vy, min_vy);
				break;
			}
			glColor3f(current_rgb[0], current_rgb[1], current_rgb[2]);
			glNormal3d(temp_v->normal.entry[0], temp_v->normal.entry[1], temp_v->normal.entry[2]);
...
void calculate_max_min(Polyhedron *this_poly)
{
	...
	{
		magnitude = sqrt((this_poly->vlist[i]->vx)*(this_poly->vlist[i]->vx) + (this_poly->vlist[i]->vy)*(this_poly->vlist[i]->vy));
		if (min> magnitude)
		{
			min = magnitude;
...
void calculate_max_min(Polyhedron *this_poly)
{
	...
	{
		magnitude = sqrt((this_poly->vlist[i]->vx)*(this_poly->vlist[i]->vx) + (this_poly->vlist[i]->vy)*(this_poly->vlist[i]->vy));
		angle = (atan2(this_poly->vlist[i]->vy, this_poly->vlist[i]->vx) * 360) / (2 * M_PI);
		...
		if (min_x > this_poly->vlist[i]->x)
		{
			min_x = this_poly->vlist[i]->x;

		}
		if (max_x < this_poly->vlist[i]->x)
		{
			max_x = this_poly->vlist[i]->x;
		}
		
		if (min_y > this_poly->vlist[i]->y)
		{
			min_y = this_poly->vlist[i]->y;

		}
		if (max_y < this_poly->vlist[i]->y)
		{
			max_y = this_poly->vlist[i]->y;
		}
...
void gen_noise_tex()
{
	for (int x = 0; x < IMG_RES; x++)
		...
		{
			noise_tex[x][y][0] = (unsigned char)255 * (rand() % 32768) / 32768.0;
			noise_tex[x][y][1] = (unsigned char)255 * (rand() % 32768) / 32768.0;
	...
	int i, j;
		...
		for (i = 0; i < this_poly->ntris; i++) {
			...
			for (j = 0; j < 3; j++)
			{
				...
				float rgb[3];
				rgb[0] = (v->vx - min_vx) / (max_vx - min_vx); // red channel
				rgb[1] = (v->vy - min_vy) / (max_vy - min_vy); // green channel
				rgb[2] = 0.5;
