...
void contour_calculation(int x, int y);
...
InitGlui( void )
{
	...
	//GLUI_Rollout *rollout = Glui->add_rollout_to_panel(panel,  "Axes Color", 1 );
		...
		myspinner1 = Glui->add_spinner("Number of Contours", GLUI_SPINNER_INT, &set_contours);
		myspinner1->set_int_limits(0, 100);
		myspinner1->set_speed(1);
		myspinner1->set_int_val(5);

		myspinner2 = Glui->add_spinner("S value", GLUI_SPINNER_INT, &set_s);
		myspinner2->set_int_limits(0, 100);
		myspinner2->set_speed(1);
		myspinner2->set_int_val(5);
...
void contour_calculation(int i, int j)
{
	
	int intersection_counter = 0;
	float s1, s2, s3, s4;
	float t_new=0, s_new=set_s;
	s1= grid_pts[(i*NX) + j].s;
	s2= grid_pts[(i*NX) + j+1].s;
	s3= grid_pts[(i*NX) + j+NX+1].s ;
	s4 = grid_pts[(i*NX) + j + NX].s ;
	...
	if (s1 > s2)
	{
		...
		if (t_new > 0 && t_new < 1)
		{
			temp.x = (1 - t_new) * grid_pts[(i*NX) + j+1].x + t_new * grid_pts[(i*NX) + j].x;
			temp.y = grid_pts[(i*NX) + j].y;
			...
			if (t_new > 0 && t_new < 1)
			{
				temp.x = (1 - t_new) * grid_pts[(i*NX) + j].x + t_new * grid_pts[(i*NX) + j + 1].x;
				temp.y = grid_pts[(i*NX) + j].y;
				intersection_counter++;
				points.push_back(temp);
			}
	...
	if (s2 > s3)
	{
		...
		if (t_new > 0 && t_new < 1)
		{
			temp.x= grid_pts[(i*NX) + j+1].x;
			temp.y = (1 - t_new) * grid_pts[(i*NX) + j+1+NX].y + t_new * grid_pts[(i*NX) + j + 1].y;
			intersection_counter++;
			points.push_back(temp);
		...
		{
			temp.x = grid_pts[(i*NX) + j + 1].x;
			temp.y = (1 - t_new) * grid_pts[(i*NX) + j + 1 ].y + t_new * grid_pts[(i*NX) + j + 1+NX].y;
			intersection_counter++;
			points.push_back(temp);
		}
	...
	if (s3 > s4)
	{
		...
		if (t_new > 0 && t_new < 1)
		{
			...
			temp.x = (1 - t_new) * grid_pts[(i*NX) + j + NX].x + t_new * grid_pts[(i*NX) + j + 1+NX].x;
			temp.y= grid_pts[(i*NX) + j + NX].y;
			intersection_counter++;
			points.push_back(temp);
		...
		if (t_new > 0 && t_new < 1)
		{
			temp.x = (1 - t_new) * grid_pts[(i*NX) + j + NX+1].x + t_new * grid_pts[(i*NX) + j + NX].x;
			temp.y = grid_pts[(i*NX) + j + NX].y;
			intersection_counter++;
			points.push_back(temp);
		}
	...
	if (s1 > s4)
	{
		...
		if (t_new > 0 && t_new < 1)
		{
			temp.x = grid_pts[(i*NX) + j + NX].x;
			temp.y = (1 - t_new) * grid_pts[(i*NX) + j + 1 + NX].y + t_new * grid_pts[(i*NX) + j].y;
			intersection_counter++;
			points.push_back(temp);
	...
	else if (s1 < s4) 
	{
		...
		{
			temp.x = grid_pts[(i*NX) + j + NX].x;
			temp.y = (1 - t_new) * grid_pts[(i*NX) + j ].y + t_new * grid_pts[(i*NX) + j + NX+1].y;
			intersection_counter++;
			points.push_back(temp);
		}
